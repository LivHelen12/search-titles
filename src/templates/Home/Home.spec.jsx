...
describe('<Home />', () => {
  ...
  it('should render search, posts and load more', async () => {
    render(<Home />);
    const noMorePosts = screen.getByText('Não existem posts com o título buscado.');

    expect.assertions(3);

    await waitForElementToBeRemoved(noMorePosts);

    const search = screen.getByPlaceholderText(/Search a title/i);
  ...
  it('should search for posts', async () => {
    ...
    userEvent.type(search, 'Não existem posts com o título buscado.');
    expect(screen.getByText('Não existem posts com o título buscado.')).toBeInTheDocument();
  });

  it('should load more posts when the button "Load More Posts is clicked"', async () => {
    render(<Home />);
    const noMorePosts = screen.getByText('Não existem posts com o título buscado.');
